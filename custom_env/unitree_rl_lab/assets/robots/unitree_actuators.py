...
class UnitreeActuator(IdealPDActuator):
    ...
    def compute(
        ...
        # tau_default = torch.from_numpy(np.array([
        #     -3.236319065093994,
        #     3.5272598266601562,
        #     -3.432962656021118,
        #     3.544980049133301,
        ...
        #     7.338108539581299,
        #     6.5652241706848145
        # ], dtype=float)).to(self.computed_effort.device)
        # control_action.joint_positions = joint_pos
        # # control_action.joint_efforts += tau_default[None, :]
        # print(control_action.joint_efforts)
        # tau = super().compute(control_action, joint_pos, joint_vel)
        # print(tau.joint_efforts)
        # control_action.joint_efforts = torch.from_numpy(np.array([
        #     0.1000, -0.1000, 0.1000, -0.1000, 0.8000, 0.8000, 1.0000, 1.0000,
        #     -1.5000, -1.5000, -1.5000, -1.5000,
        # ], dtype=float)).to(self.computed_effort.device)
        # # control_action.joint_efforts = None
        # control_action.joint_positions = None
        # control_action.joint_velocities = None
        # print("controller: ", control_action.joint_positions)
