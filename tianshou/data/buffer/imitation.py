...
class ImitationReplayBuffer:
    ...
    @classmethod
    def from_data(
        cls, obs: h5py.Dataset, act_oracle: h5py.Dataset,
    ) -> "ReplayBuffer":
        size = len(obs)
        assert all(len(dset) == size for dset in [obs, act_oracle]), \
    ...
    def _add_index_n(self, n):
        """Maintain the buffer's state after adding n data batch.

        Return (index_to_be_modified, episode_reward, episode_length,
        episode_start_index).
        """
        self.last_index[0] = ptr = self._index
        self._size = min(self._size + n, self.maxsize)
        self._index = (self._index + n) % self.maxsize
    ...
    def addn(
        ...
        ptr = self._add_index_n(n)
        try:
            self._meta[ptr:ptr+n] = batch
        except ValueError:
            stack = True
            if self._meta.is_empty():
                self._meta = _create_value(  # type: ignore
    ...
    def __getitem__(self, index: Union[slice, int, List[int], np.ndarray]) -> Batch:
        ...
        return Batch(
            ...
            act_oracle=self.act_oracle[indices],
            d_obs=self.get(indices, "d_obs", Batch()),  ## TODO:new
            d_act=self.get(indices, "d_act", Batch()),  ## TODO:new
            d_bool=self.get(indices, "d_bool", Batch()),  ## TODO:new
        )
