...
class ReplayBuffer:
    ...
    def reset_length(self, length, keep_statistics: bool = False) -> None:
        """Clear all the data in replay buffer and episode statistics."""
        self.last_index = np.array([length])
        self._index = self._size = length
        if not keep_statistics:
            self._ep_rew, self._ep_len, self._ep_idx = 0.0, 0, 0
    ...
    def __getitem__(self, index: Union[slice, int, List[int], np.ndarray]) -> Batch:
        ...
        return Batch(
            ...
            act_oracle=self.get(indices, "act_oracle", Batch()),  ## TODO:new
